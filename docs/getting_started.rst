...
Dynamic JWT can be installed with pip::

  pip install djangorestframework-simplejwt

Then, your django project must be configured to use the library.  In
``settings.py``, add
``rest_framework_dynamicjwt.authentication.JWTAuthentication`` to the list of
...
.. code-block:: python
  ...
  REST_FRAMEWORK = {
      ...
      'DEFAULT_AUTHENTICATION_CLASSES': (
          ...
          'rest_framework_dynamicjwt.authentication.JWTAuthentication',
      )
      ...
  }

Also, in your root ``urls.py`` file (or any other url config), include routes
for Dynamic JWT's ``TokenObtainPairView`` and ``TokenRefreshView`` views:
...
You can also include a route for Dynamic JWT's ``TokenVerifyView`` if you wish to
...
.. code-block:: python
  ...
  from rest_framework_dynamicjwt.views import TokenVerifyView
...
``rest_framework_dynamicjwt`` to ``INSTALLED_APPS``.
...
To verify that Dynamic JWT is working, you can use curl to issue a couple of
test requests:
